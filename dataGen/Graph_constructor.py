...
if __name__ == '__main__':
    ...
    ''' 2. write into point cloud file '''
    file = open(config.dir_meshlab + "fm_{}.txt".format(0),'w')
    for i in range(config.pts_size):
    ...
    for layer in range(1, 10):
        ...
        ''' 2. write into point cloud file '''
        file = open(config.dir_meshlab + "fm_{}.txt".format(layer),'w')
        for i in range(config.pts_size):
            file.write('%.16f\t%.16f\t%.16f\n' % (cur_points[i][0], cur_points[i][1], cur_points[i][2]))
