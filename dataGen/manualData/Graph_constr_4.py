...
import shutil
import random
from Graph_config_4 import ClearDir, Configuration
...
def GetPointSets():  # 获得最初的point set（包含num_clusters个簇，每个簇
    ...
    # _points.append(np.random.uniform(
    #     size=config.pts_size//config.num_clusters, low = _mean + , high=_mean + ))
    # _labels.append([0 for t in range(config.pts_size//config.num_clusters)])
...
# def scaleCluster(inputs, labels, shrink_label, new_center):
#     output = inputs
#     ids = range(0, config.pts_size)
...
#     _cov = np.diag(np.array([config.gauss_delta_/4. for i in range(config.dim)]))  # 方差变小，则面积变小
#     _points = np.random.multivariate_normal(size=config.pts_size//config.num_clusters, mean=new_center, cov=_cov)

#     assert(len(shrink_ids) == len(_points))

#     # 将重新生成的点覆盖这个簇
#     for i in range(len(_points)):
#         output[shrink_ids[i]] = _points[i]

#     return output


# 缩小整个类簇而不改变knn
def scaleCluster(inputs, labels, shrink_label, new_center, scale_factor = 0.25):
    output = inputs
    ids = range(0, config.pts_size)

    # 先算出该类簇中心
    scale_center = cluster_center(inputs, labels, shrink_label)
...
def cluster_center(inputs, labels, label):
    ids = range(0, config.pts_size)

    center = np.zeros((config.dim))
    count = 0
    for id in ids:
        if labels[id] == label:
            center += inputs[id, :]
...
if __name__ == '__main__':
    ...
    for layer in range(1, 2):
        ...
        # 随机产生两个类标签
        [split_label, shrink_label] = random.sample(range(config.num_clusters), 2)
        print("select:{}, {}".format(split_label, shrink_label) )

        # first split one cluster
        cur_points = splitClusters(points, labels, split_label)

        # caculate the center of splitted cluster
        split_center = cluster_center(cur_points, labels, split_label)

        # then move one cluster into the center of 
        cur_points = scaleCluster(cur_points, labels, shrink_label, split_center)


        add_noise_ids = [id for id in range(config.pts_size) if  labels[id] != split_label and labels[id] != shrink_label]
        cur_points = addNoise(cur_points, noise_intensity = config.half_space_dist_/2., add_noise_ids= add_noise_ids)
