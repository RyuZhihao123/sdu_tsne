...
import shutil
import random
from Graph_config_2 import ClearDir, Configuration
...
def DisturbClusters(inputs, labels, disturb_labels = []):
    ...
    for i in dist_ids:
        output[i] += _moveVec
    return output, keep_ids, dist_ids
...
if __name__ == '__main__':
    ...
    for layer in range(1, 10):
        ...
        disturb_labels = random.sample(range(config.num_clusters), disturb_cluster_num)
        cur_points, keep_ids, dist_ids= DisturbClusters(points, labels, disturb_labels)

        # 对移动的簇进行大范围扰动
        cur_points = addNoise(inputs = cur_points, add_noise_ids = dist_ids, noise_intensity = config.half_space_dist_/2.)

        # 对其他点进行小扰动
        cur_points = addNoise(inputs = cur_points, add_noise_ids = keep_ids, noise_intensity = config.half_space_dist_/10.)
